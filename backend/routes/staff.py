...
from db.schema import StaffCreate, StaffResponse
...
async def update_staff_status(staff_id: int, update: StaffStatusUpdate, db: AsyncSession = Depends(get_db)):
    return await update_staff_status_controller(db, staff_id, update)

@router.post("/{staff_id}/auto-assign")
async def auto_assign_orders(staff_id: int, db: AsyncSession = Depends(get_db)):
