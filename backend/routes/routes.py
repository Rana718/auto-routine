...
from db.schema import RouteStatus
...
async def generate_route(data: RouteGenerate, db: AsyncSession = Depends(get_db)):
    return await generate_route_controller(db, data)

@router.post("/regenerate-all")
async def regenerate_all_routes(route_date: date = None, db: AsyncSession = Depends(get_db)):
    return await regenerate_all_routes_controller(db, route_date)

@router.patch("/{route_id}/status")
async def update_route_status(route_id: int, status: RouteStatus, db: AsyncSession = Depends(get_db)):
    return await update_route_status_controller(db, route_id, status)

@router.patch("/{route_id}/stops/{stop_id}")
async def update_stop(route_id: int, stop_id: int, update: StopUpdate, db: AsyncSession = Depends(get_db)):
    return await update_stop_controller(db, route_id, stop_id, update)

@router.post("/start-all")
async def start_all_routes(route_date: date = None, db: AsyncSession = Depends(get_db)):
